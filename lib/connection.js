...
Connection.prototype.sendRPC = function(method, params, callback) {
  ...
  timer = setTimeout(function() {
    ctx.pendingRpc[id].callback(new Error('Send RPC time out, ', id, method, params));
  }, TIMEOUT);

  this.pendingRpcCount++;
  if (this.ended) this.pendingRpc[id].callback(new Error('Client have been closed!'));
  else if (this.connected) this.socket.write(this.pendingRpc[id].data);
