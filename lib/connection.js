...
var RpcCommand = require('./rpcCommand')
...
Connection.prototype.send = function (params) {
  this.messagesCount++
  this.rpcRequestPool.push(params)
  this.execQueue()
}
...
Connection.prototype.execQueue = function () {
  if (this.pendingRpcCount <= this.RPC_HIGH_WATER) {
    while (this.rpcRequestPool.length) this.sendRpc('publish', this.rpcRequestPool.splice(0, this.MESSAGE_HIGH_WATER))
  }
}
...
Connection.prototype.close = function () {
  this.destroy()
  this.ended = true
  for (var id in this.pendingRpc)
    this.pendingRpc[id].done(new Error('Client have been closed!'))
}
